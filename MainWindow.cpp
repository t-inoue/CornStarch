...
CMainWindow::CMainWindow(void) :
		m_view(NULL), m_logHolder(NULL), m_contents(NULL)
{
}
...
void CMainWindow::updateMessageView(const wxString& channel)
{
	...
	// メッセージを表示
	m_view->displayMessages(m_contents->getMessages(channel),
			m_contents->getNickTable());
}
...
void CMainWindow::onRegister(wxCommandEvent& event)
{
	...
	// コンテンツを更新
	m_contents->registerUser(m_view->getDlgUserNameAsString(),
			m_view->getDlgPasswordAsString());
}
...
void CMainWindow::onMsgStream(CMsgStreamEvent& event)
{
	...
	if (!myPost)
	{
		m_logHolder->pushMessageLog(data,
				m_contents->getMemberNick(data.m_username));
	}
...
void CMainWindow::onJoinStream(CJoinStreamEvent& event)
{
	...
	if (event.getUserName() == m_contents->getNickName())
	{
		return;
	}
	// 処理待ちに追加
