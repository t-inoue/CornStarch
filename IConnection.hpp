...
public:
	...
	virtual void startGetChannelTask(wxEvtHandler* handler,
			const std::string& userName, const std::string& basic) = 0;

	// チャンネルから離脱するタスク(別スレッド)を開始する
	virtual void startPartTask(wxEvtHandler* handler,
			const std::string& channel, const std::string& userName,
			const std::string& basic) = 0;
